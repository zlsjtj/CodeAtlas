...
def list_repo_tree(payload: ListRepoTreeRequest, db: Session = Depends(get_db)) -> ToolExecutionResponse:
...
def search_repo(payload: SearchRepoRequest, db: Session = Depends(get_db)) -> ToolExecutionResponse:
    tools = RepositoryTools(db)
    try:
...
def read_file(payload: ReadFileRequest, db: Session = Depends(get_db)) -> ToolExecutionResponse:
    tools = RepositoryTools(db)
    try:
...
def find_symbol(payload: FindSymbolRequest, db: Session = Depends(get_db)) -> ToolExecutionResponse:
    tools = RepositoryTools(db)
    try:
